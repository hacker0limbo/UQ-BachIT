...
def login():
    ...
    if 'GET' == m:
        ...
        if u is None:
            # 登录失败, 使用 flash
            flash(u'Invalid password or username', 'danger')
...
def profile():
    ...
    if u is None:
        ...
        return render_template('portrait.html', user=u)
...
def setting():
    u = current_user()
    return render_template('setting.html', u=u, bid=-1)
...
@main.route('/search/topic', methods=['POST'])
def search_topic():
    topic_title = request.form.get('topicTitle')
    topic = Topic.one(title=topic_title)
    if topic is None:
        abort(404)
    else:
        return redirect(url_for('topic.detail', id=topic.id))
